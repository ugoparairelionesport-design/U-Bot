...
console.log('🚀 [configsystem.js] Loading version 2.6.4...');
...
async function resumeTicketState(client) {
  ...
  console.log(`🔍 [SYSTEM - TICKETS VER: 2.6.4] Analyse et restauration pour ${Object.keys(configData.guilds).length} serveur(s)...`);
...
async function handleButtons(interaction) {
  try {
    ...
    if (interaction.customId === 'claim_ticket') {
      ...
      if (guildConfig.claims[interaction.channel.id]) return replyAndAutoDelete(interaction, { content: "❌ Déjà pris", flags: 64 });
